 ...
 */
func getSavedMeals() -> [String: Meal] {
    let mealLog: [String: Any]? = UserDefaults.standard.dictionary(forKey: "mealLog")
    if let mealLog = mealLog as? [String: Meal]{
        return mealLog
    }
    else{
        return [:]
